...
casilla Ajedrezbot::Click_inicio()
{
    ...
    // Al seleccionar una pieza calcula todos sus movimientos legales, en caso de no haber, se elimina la selección
...
void Ajedrezbot::Click_fin(casilla o)
{
    casilla seg = raton;
    ...
    if (encontrar == true)
    {
        ...
        else
        {
            ...
            // Si es el turno de las negras y no hay jaque mate, la IA juega su turno
            if (turno == Pieza::NEGRA && !jaque_mate) {
                turnoIA();
            }
        }
    }
...
bool Ajedrezbot::compruebaJaque(Tablerobot& tablero, Pieza::COLOR color)
{
...
bool Ajedrezbot::compruebaJaqueMate(Pieza::COLOR color)
{
    vaciarMovimientos_legales();
    for (int f = 0; f < NumFilas; f++)
    {
...
void Ajedrezbot::calcularMovimientos_legales(casilla o, Pieza::COLOR color)
{
    ...
    for (auxiliar.fila = 0; auxiliar.fila < NumFilas; auxiliar.fila++)
    {
        for (auxiliar.columna = 0; auxiliar.columna < NumColumnas; auxiliar.columna++)
        {
            ...
            {
                Tablerobot tablero_aux = tablero;
...
void Ajedrezbot::vaciarMovimientos_legales()
{
    for (int i = 0; i < num_movimientos_legales; i++)
    {
...
void Ajedrezbot::detectaClick(int x, int y, int button, bool down)
{
    GLint viewport[4];
...
void Ajedrezbot::turnoIA()
{
    if (turno == Pieza::NEGRA && !jaque_mate)
    {
        int indiceMovimiento = rand() % num_movimientos_legales; // Selecciona un movimiento aleatorio
        casilla o = *movimientos_legales[indiceMovimiento]; // Selecciona la casilla de inicio del movimiento
        Click_fin(o);
    }
}
